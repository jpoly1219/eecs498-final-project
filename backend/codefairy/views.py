...
def create_prompt(user_code, test_input, user_output, expected_output, result):
    ...
    prompt = "\n".join([
        ...
        "```",
        "",
        f"The `error_lines` field should be a list of line numbers where the error happened. The error cannot occur in line 1 and 2, because they are pre-given by me. Where the error happened depends on what step your student's code is missing, and your student's output {user_output} for an input {test_input}, whereas the expected output is {
        ...
        "",
        "The `feedback` field should include what step name in the expert step-by-step solution your student's code is missing, and a short feedback on how your student can improve their code. When you are saying what step name, you should say the step's name rather than the number. For example, say 'Your solution doesnt handle base cases correctly', not 'You are missing step 4'. You should say in your response what lines are responsible for the missing step, and say why. The lines you say here must correspond to the lines in the `error_lines` field. Your student's code can be missing multiple steps, one step, or no steps. If your student's code is a correct dynamic programing solution, reply with 'Awesome work! Your code is correct.'. DO NOT include anything else in this field. If your student's code has a syntax error, just say that the code has a syntax error.",
        "",
